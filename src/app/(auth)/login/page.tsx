...
export default function LoginForm() {
  ...
  const handleAppleSuccess = async (response: any) => {
    // response.authorization.id_token is the JWT Apple returns
    const idToken = response?.authorization?.id_token;

    if (!idToken) {
      toast.error("Apple login failed: no token received");
      return;
    }
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/auth/apple-login`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ token: idToken }), // ← exactly what your backend needs
        },
      );
      ...
      if (res.ok && (data?.success || data?.data)) {
        ...
        dispatch(userTrack());
        dispatch(setUser({ user: data?.data?.user, token }));
  ...
  const handleAppleError = (err: any) => {
    // user_cancelled_authorize = user closed the popup — silent, no toast
        ...
        <div className='mt-8 pt-8 border-t border-gray-200'>
          ...
          <div className='flex items-center gap-6 justify-center'>
            ...
            <AppleLogin
              ...
              callback={handleAppleSuccess} // ← errors come here too, no onError prop
              render={(props) => (
                <button
                  type='button'
                  onClick={props.onClick}
                  disabled={appleLoading}
                  className='w-12 h-12 rounded-full bg-black hover:bg-gray-900 flex items-center justify-center transition shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed'
