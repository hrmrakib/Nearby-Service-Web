...
export default function DashboardLayout() {
  ...
  const { data, isFetching, refetch } = useGetAllPostQuery({
    ...
    // Pass the resolved location: prefer coordinates chosen in the filter,
    // otherwise fall back to the user's detected position.
    ...
    rating: appliedFilters.rating > 0 ? appliedFilters.rating : undefined,
    search,
    page,
    limit,
  });
  ...
  const handleApplyFilters = () => {
    setAppliedFilters({
      lat: coordinates?.lat ?? userLat ?? undefined,
      lng: coordinates?.lng ?? userLng ?? undefined,
      ...
      rating: minStarRating[0] ?? 0,
    });

    // Reset pagination so results refresh from page 1
    setPage(1);
    setHasMore(true);
    setAllPosts([]);
  };
...
//     lat: coordinates?.lat ?? userLat,
//     lng: coordinates?.lng ?? userLng,
//     // maxDistance: distanceRadius[0],
//     // minPrice: minPrice[0],
//     // maxPrice: maxPrice[0],
//     // date: "",
