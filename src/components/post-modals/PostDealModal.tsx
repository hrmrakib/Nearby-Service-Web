...
export default function PostDealModal({
  ...
  useEffect(() => {
    ...
    // Split startDate into date + time
    if (data.startDate) {
      setDate(data.startDate.slice(0, 10));
      setTime(data.startDate.slice(11, 16));
    }

    // Split endDate into date + time
    if (data.endDate) {
      setEndDate(data.endDate.slice(0, 10));
      setEndTime(data.endDate.slice(11, 16));
    }
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      if (isEditMode) {
        ...
        res = await createDealPostMutation(formData).unwrap();
      }
      ...
      <DialogContent className='sm:max-w-lg p-0 max-h-[90vh] overflow-y-auto scrollbar'>
        <DialogHeader className='flex flex-row items-center justify-between p-4 border-b'>
          <DialogTitle className='text-lg font-semibold'>
            {isEditMode ? "Edit Deal" : "Post Deal"}
          </DialogTitle>
        </DialogHeader>
